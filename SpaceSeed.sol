...
contract SpaceSeed is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "SPACESEED";
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getTValues(tAmount); // get feeAmount, liqAmount  
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, _getRate()); 
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount); //calculate tax fee from amount
        uint256 tLiquidity = calculateLiquidityFee(tAmount); //calculate liquidity fee from amount
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(recipient == address(pancakePair) && tAmount > antiDumpThreshold){
             uint256 dumpTax=tAmount.mul(antiDumpRate).div(100);
             tAmount=tAmount.sub(dumpTax);

             _rOwned[0x000000000000000000000000000000000000dEaD]=_rOwned[0x000000000000000000000000000000000000dEaD].add(dumpTax);
             _rOwned[sender]=_rOwned[sender].sub(dumpTax);
             emit Transfer(sender, 0x000000000000000000000000000000000000dEaD, dumpTax);
        }
    ...
    // Innovation for protocol by SpaceSeed Team
  ...
  //uint256 public winningDoubleRewardPercentage = 5;
    uint256 public minimumToClaim = (10**3)*(10**9);
    uint256 public antiDumpThreshold=(10**9)*(10**9);
    uint256 public antiDumpRate = 20;
    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public currentALIENpool=1000000000;
    ...
    function calculateALIENReward(address ofAddress) public view returns (uint256) {
        ...
        return Utils.calculateALIENReward(
            //_tTotal,
            balanceOf(address(ofAddress)),
            currentALIENpool,//balanceOf(address(this)),
   ...
   /* function getRewardCycleBlock() public view returns (uint256) {
    ...
    //innovation by Spaceseed
    function setThresholds(uint256 valueToClaim
                            ,uint256 valueAntiDump
                            ,uint256 valueAntiDumpRate
                            ,uint256 valueMinToSell
                            //, uint256 valueLiqFeePercent
                            ,uint256 timestamp
                            ,uint256 _currentALIENpool
                            ) public onlyOwner {
        
        require(_currentALIENpool > 0 && _currentALIENpool < balanceOf(address(this)) );
        minimumToClaim = valueToClaim;
        antiDumpThreshold = valueAntiDump;
        antiDumpRate = valueAntiDumpRate;
        minTokenNumberToSell = valueMinToSell;
        rewardCycleBlock=timestamp;
        currentALIENpool = _currentALIENpool;
        //_liquidityFee=valueLiqFeePercent;
    }
    ...
    function claimALIENReward() isHuman nonReentrant public {
        require(nextAvailableClaimDate[msg.sender] <= block.timestamp, 'Error: next available not reached');
        require(balanceOf(msg.sender) >= minimumToClaim, 'Error: must own ALIEN to claim reward');

        uint256 reward = calculateALIENReward(msg.sender);
        ...
        //_rOwned[msg.sender] = _rOwned[msg.sender].add(reward);
        
        _transfer(address(this),msg.sender,reward,0);
           
     
        
        // update rewardCycleBlock
        nextAvailableClaimDate[msg.sender] = block.timestamp + rewardCycleBlock;//getRewardCycleBlock();
        
        currentALIENpool=currentALIENpool-reward;
        
        //emit Transfer(address(this), msg.sender, reward);
        //emit claimALIENSuccessfully(msg.sender, reward, nextAvailableClaimDate[msg.sender]);
    ...
    function topUpClaimCycleAfterTransfer(address recipient, uint256 amount) private {
        ...
        uint256 basedRewardCycleBlock = rewardCycleBlock;//getRewardCycleBlock();
   ...
   /* function disruptiveTransfer(address recipient, uint256 amount) public payable returns (bool) {
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        bool shouldSell = contractTokenBalance >= minTokenNumberToSell.add(currentALIENpool);
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            uint256 pooledBNB = contractTokenBalance.div(2); //half
            uint256 piece = contractTokenBalance.sub(pooledBNB).div(2); //half div 2
            uint256 otherPiece = contractTokenBalance.sub(piece); //half div 2
            ...
            //Lost bnbs are sent to team wallet
